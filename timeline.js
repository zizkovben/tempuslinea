...
window.TimelineEngine = (() => {
  ...
  function tickInterval() {
    ...
    // buckets scaled to match, this fell through to the 20,000-year bucket
    // below and produced ~690,000 grid-tick loop iterations (each doing real
    // canvas draw calls, twice — once for gridlines, once for axis labels),
    // which froze the page. These buckets extend the same logarithmic
    // scaling pattern up to cosmological spans.
    if (sp > 5000000000) return 1000000000;
    if (sp > 500000000)  return 100000000;
    if (sp > 50000000)   return 10000000;
    if (sp > 5000000)    return 1000000;
    if (sp > 500000)     return 100000;
  ...
  function resize() {
    ...
    // of years) lane-packs the FULL uncapped dataset just to size the
    // canvas — producing a huge lane count and a canvas potentially tens
    // of thousands of pixels tall, which freezes the page.
    const vis = getVisible();
    const pxPerYearSizing = (CW / Math.max(vE - vS, 1));
    let sizingSet = vis;
    if (pxPerYearSizing < 0.001 && vis.length > 600) {
      sizingSet = vis.filter(c => c.t === 'confirmed' || (c.up||0) > 500);
    } else if (pxPerYearSizing < 0.01 && vis.length > 900) {
